...
function hasPath(turns) {
  return /coffee shop|cafe|café|owner|barista|church|pastor|missionar|on the field|abroad/.test(
    answersBlob(turns)
  );
}
...
function answerTo(turns, topic) {
  const hit = [...turns].reverse().find((t) => t.a && t.q === topic.ask);
  return hit ? hit.a : "";
}

function hasAnswered(turns, topic) {
  return Boolean(answerTo(turns, topic));
}

function nextTopic(turns, contact) {
  if (!hasPath(turns)) return TOPICS[0];
  if (!hasAnswered(turns, TOPICS[1])) return TOPICS[1];
  if (!hasAnswered(turns, TOPICS[2])) return TOPICS[2];
  if (!hasAnswered(turns, TOPICS[3])) return TOPICS[3];
...
function extractContact(turns, parsed) {
  ...
  const named = answerTo(turns, TOPICS[4]);
  const mailed = answerTo(turns, TOPICS[5]);
  const phoned = answerTo(turns, TOPICS[6]);
  const email =
    (parsed && parsed.email) ||
    mailed ||
  ...
  const name = String(
    (parsed && parsed.name) ||
      (named && !/@/.test(named) && named.length < 80 ? named : "")
  )
    .trim()
    .slice(0, 80);
  return {
...
function lockQuestion(parsed, topic) {
  ...
  return { question: topic.ask, chips: topic.chips || null };
}
...
async function logSheet(payload) {
  ...
  if (!/^\s*\{/.test(body)) {
    throw new Error("Sheets returned a login page instead of JSON");
  }
  let data = {};
  try {
    data = JSON.parse(body);
  } catch {
    throw new Error("Sheets returned non-JSON");
  }
  if (!data.ok) {
    throw new Error("Sheets rejected write: " + (data.error || "unknown"));
  }
  return { ok: true };
}
